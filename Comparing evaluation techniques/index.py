...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        0.444...
        """
        norm = defaultdict(lambda: 0.0)
        for token in index.keys():
            term_idf = doc_freqs[token]
            for values in index[token]:
                norm[values[0]] += (((1 + math.log10(values[1])) * 1.0 * math.log10(n_docs/term_idf))**2)
        
        for key in norm.keys():
            norm[key] = math.sqrt(norm[key])
    ...
    def compute_doc_lengths(self, index):
        ...
        for key in index.keys():
            for values in index[key]:
                res[values[0]] = res[values[0]] + values[1] * 1.0
                mean += values[1]
        
        return (res,float(mean/len(res.keys())))
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        for doc_id in range(len(docs)):
            for token in docs[doc_id]:
                term_freq[token][doc_id+1] += 1 
        
        res = defaultdict(lambda: [])
        for token in term_freq.keys():
            for docid in term_freq[token].keys():
                res[token].append([docid,(term_freq[token][docid] * 1.0)])
    ...
    def count_doc_frequencies(self, docs):
        ...
        res = defaultdict(lambda: 0)
        for key in doc_term_freq.keys():
            res[key] = len(doc_term_freq[key].keys()) * 1.0 
    ...
    def query_to_vector(self, query_terms):
        ...
        for qry_term in query_terms:
            if qry_term in self.index:
                res[qry_term] = float(math.log10( N * 1.0 / self.doc_freqs[qry_term]))
